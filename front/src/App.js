...
function UserInfo({name, age, updateUserInfo}) {
...
function UserInfo2({name, age, updateUserInfo}) {
...
const UserInfoMemo = React.memo(props => {
   return (
      <UserInfo name={props.name} age={props.age} updateUserInfo={props.updateUserInfo}></UserInfo>
   )
})

const UserInfoMemo2 = React.memo(props => {
   return (
      <UserInfo2 name={props.name} age={props.age} updateUserInfo={props.updateUserInfo}></UserInfo2>
   )
})

function App() {
   const [number, setNumber] = useState(1);
   const [userInfo, setUserInfo] = useState({name: 'test1', age: 20});
   ...
   const updateUserCallback = useCallback((targeNmae, targeAge) => {
      updateUser((targeNmae, targeAge), [targeNmae, targeAge])
   })
   return (
      <div>
